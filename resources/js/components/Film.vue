<template>
      ...
      <div class="container-fluid" v-for="evento_pelicula in evento_peliculas" :key="evento_pelicula.id">
        ...
        <div class="cartelera-fluid">
          <div class="cartelera-film" v-for="pelicula_nueva in evento_pelicula.evento_pelicula_nueva" :key="pelicula_nueva.id" :style="{
            ...
            <div id="hora-film">
              <a v-for="horario in horarios" :key="horario" :href="esHorarioDisponible(horario, evento_pelicula.fecha_inicio) ? `ticket/${pelicula_nueva.id}/${horario}` : '#'"
            :class="['horario-link', { 'disabled': !esHorarioDisponible(horario, evento_pelicula.fecha_inicio) }]">
                {{ horario }}
              </a>
            </div>
          </div>
  ...
  export default {
    ...
    const diaSeleccionadoIndex = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'].indexOf(this.diaSeleccionado.toLowerCase());
    const fechaSeleccionada = new Date();
    fechaSeleccionada.setDate(fechaSeleccionada.getDate() + (diaSeleccionadoIndex - fechaSeleccionada.getDay()));

    const horaActual = new Date().getHours();

    if (fechaSeleccionada.toLocaleDateString() === new Date().toLocaleDateString()) {
  ...
  padding: 10px 20px;
  margin: 5px;
  background-color: #1f1e24; /* Color base oscuro */
  color: white;
...
.horario-link.disabled {
  background-color: #f0f0f0; /* Color más blanco */
  color: #999;
  cursor: not-allowed;
}

.horario-link:not(.disabled):hover {
  background-color: #3a3a3a; /* O un tono que combine */
  cursor: pointer;
}
